  ...
  <div :class="cn('flex flex-col', props.class)">
    ...
    <div v-if="posterSrc || synopsis" class="flex gap-4 px-5 mt-4">
      <div v-if="posterSrc" class="shrink-0 w-[132px] h-[195px] rounded-sm border border-foreground/10 overflow-hidden">
        <img v-fade-on-load loading="lazy" decoding="async" :src="posterSrc" :alt="posterAlt" class="size-full object-cover">
      </div>
      <p v-if="synopsis" class="flex-1 font-content text-sm text-foreground leading-normal overflow-hidden">
        {{ synopsis }}
      </p>
    </div>
